...
def load_models():
    ...
    net.eval()
    net_param = torch.load('weights/trainer_5000.pth')

    # Remove self sup train headers
    new_net_param = {}
    ...
    pre_processor.eval()
    pre_processor.load_state_dict(torch.load('weights/Illumination-Enhancer.pth'))
...
def vis_detections(im,  dets, image_name , thresh=0.5):
    ...
    if len(inds) == 0:
        return
    print (f'Numner of faces detected = {len(inds)}')
    im = im[:, :, (2, 1, 0)]
    ...
    plt.axis('off')
    plt.tight_layout()
    plt.savefig(args.save_folder+image_name+'_'+class_name, dpi=fig.dpi)
    print(f'Output is saved in "{args.save_folder}" directory')


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='DSFD + DCENet + MOCO2')

    parser.add_argument('--save_folder', default='result/', type=str, help='Dir to save results')
    parser.add_argument('--visual_threshold', default=0.6, type=float, help='Final confidence threshold')
    parser.add_argument('--cuda', default=True, type=bool, help='Use cuda to train model')
    parser.add_argument('--path_to_img', default='test.png', help='Path of the image to run demo')
    # parser.add_argument('--label_path', default='labels.txt', help='Path to true labels')
    ...
    with torch.no_grad():
        enhanced_image = pre_processor(image_to_enhance)

    image = tensor_to_image(enhanced_image)
    ...
    vis_detections(image , dets , img_id, args.visual_threshold)
