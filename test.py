...
def load_models():
    ...
    # net.load_state_dict(torch.load('weights/Face-Detector.pth'))
...
def load_images(test_data_path):
    if test_data_path  == None:
        return ['./test.png']
        
    return glob.glob(f'{test_data_path}/images/*.png') 
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_path', default='result/', type=str, help='Dir to save results')
    parser.add_argument('--visual_threshold', default=0.6, type=float, help='Final confidence threshold')
    parser.add_argument('--cuda', default=True, type=bool, help='Use cuda to train model')
    parser.add_argument('--test_path', default=None, help='Path of test dataset')
    ...
    img_list = load_images(args.test_path)
    ...
    for img_path in tqdm(img_list):
        ...
        # Save result
        fout = open(os.path.join(args.save_path, Path(os.path.basename(img_path)).stem + '.txt'), 'w')
